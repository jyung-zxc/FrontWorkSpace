test1.onclick = function () {
    ...
    */
    ...
    //  - 브래킷표기법은 속성명을 
    //    반드시 문자열 리터럴 혹은 문자열 변수를 제시해야한다.
    obj2['origin'] = '필리핀';

    // 문자열에 특수문자나 , 띄어쓰기 등이 포함된 경우 대괄호 표기법 사용
    obj2['ty-pe'] = '피클';
...
test2.onclick = function () {
    // 자바에서는 class안에 선언한 함수를 메서드라고 불렀는데.
...
test3.onclick = function () {
    game = {
        ...
        duration: function () {
            // 두시간의 차이를 ms로 반환
            var dur = this['endtime'] - this['start-time'];
            dur = Math.floor(dur / 1000 / 60);
            console.log(dur + "분 동안 풀레이 하셨네요.")
        }
    };
    ...
    for (var key in game) {
        ...
        // console.log("value : " + game.key); // 점표기법으로는 조회불가
...
//객체는 생성된 이후, 자유롭게 속성을 "추가" 하거라, 삭제할 수 있다.
...
test7.onclick = function () {
    ...
    */
    function Pet(kind, name, breed, weight) {
        //필드부
        // - 생성자 함수 내부에서는 this는 현재 생성된 객체의 주소값을 의미한다.
        this.kind = kind;
        this.name = name;
        this.breed = breed;
        this.weight = weight;
...
test8.onclick = functionint(){
    ...
    */
    ...
    class Pet extends animal {
        constructor(kind, came, breed, weight) {
            super(kind);
            this.name = this.name;
            this.breed = breed;
            this.weight = weight;
